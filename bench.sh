...
then
    ...
    perf record --call-graph=dwarf _build/default/src/bin/native.exe >/dev/null 2>&1
elif [ "$1" = "report" ]
then
    perf report
elif [ "$1" = "ref" ]
then
    dune exec src/bin/native.exe  > "$BASE_DIR"/timing.ref
    exit 0
elif [ "$1" = "diff" ]
then
     dune exec --display=quiet -- src/bin/native.exe > "$BASE_DIR"/timing.tmp
